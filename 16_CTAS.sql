...
CREATE TABLE patient_detail AS
       
       SELECT patient_id, name, gender, age, city, disease
       FROM PATIENTS;
...
CREATE TABLE patient_age40 AS 

         SELECT patient_id, name, age, disease
         FROM patients;
         
DROP TABLE patient_age40;

CREATE TABLE patient_age40 AS 

        SELECT patient_id, name, age, disease, city
        FROM patients  
        WHERE age > 40;
...
CREATE TABLE patients_appointment_table AS
   
       SELECT p.patient_id, p.name, p.age, p.disease, p.city,
...
CREATE TABLE patient_disease_count AS
...
CREATE TABLE statusofappointments AS 
...
CREATE TABLE diseases_appointments AS 
