...
WITH 
    medicine_overall_average 
    AS
    (
    SELECT 
          AVG(price) overall_average
    FROM medicines
    )
    ,
    medicine_category_average 
    AS 
    (
    SELECT 
          category, 
    ...
    WHERE 
          mca.category_average > moa.overall_average
    )
