...
SELECT patient_id,
       name,
       age,
      ROW_NUMBER() OVER(ORDER BY age DESC) row_number
FROM patients;
...
SELECT name,
       disease,
       age,
       RANK() OVER(PARTITION BY disease ORDER BY age) disease_rank
FROM patients;
...
SELECT name,
       city,
       age,
       DENSE_RANK() OVER(PARTITION BY city ORDER BY age) dense_rank_age
FROM patients;
...
SELECT p.patient_id,
       p.disease,
       p.name,
       COUNT(pr.prescription_id) OVER(PARTITION BY p.patient_id) AS patient_prescription_count,
       COUNT(pr.prescription_id) OVER(PARTITION BY p.disease) AS disease_prescription_count
FROM patients p
...
SELECT pr.prescription_id,
       ...
       COUNT(pr.prescription_id) OVER(PARTITION BY p.patient_id ORDER BY pr.prescription_date) AS running_count
FROM patients p
JOIN prescriptions pr
ON p.patient_id = pr.patient_id;
