...
SELECT  
       ...
       DATE_PART('Day', appointment_date) AS appointment_day,
       DATE_PART('Month', appointment_date) AS appointment_month,
       TO_CHAR(appointment_date, 'MMMM') AS appointment_month2,
       DATE_PART('Year', appointment_date) AS appointment_year
...
SELECT 
       DATE_TRUNC('Month', appointment_date) month_truncate,
...
SELECT 
       ...
       lb.test_date,
       a.appointment_date,
       ABS(DATEDIFF(DAYS, lb.test_date, a.appointment_date)) AS date_difference
...
SELECT 
       DATE_TRUNC(QUARTER, appointment_date) quarter_appointments,
       COUNT(appointment_id) total_appointments
       
FROM appointments
GROUP BY 
       DATE_TRUNC(QUARTER, appointment_date);
...
WHERE 
       DATEDIFF(DAYS, a.appointment_date, pr.prescription_date)
       BETWEEN 0 AND 7;

-- Q8. DATEDIFF + AVG: Find the average number of days
--      between a patient's appointment date and lab test date.
SELECT 
       ABS(AVG(DATEDIFF(DAY, lt.test_date, a.appointment_date))) average_days
